/* Authorization Letter fixed Legal page + automatic pagination. */
(function () {
  'use strict';
  ...
  function restoreSelection(root, saved) {
    if (!saved) return;
    try {
      const start = textPoint(root, saved.start), end = textPoint(root, saved.end);
    ...
    const makeSplit = (offset) => {
      ...
      aTexts.forEach((node, i) => {
        ...
        if (consumed >= offset) node.nodeValue = '';
          ...
          node.nodeValue = original.slice(0, cut); if (bNode) bNode.nodeValue = original.slice(cut);
  ...
  function moveOverflow(page, nextPage) {
    ...
    while (page.scrollHeight > page.clientHeight + 1 && guard++ < 500) {
      ...
      if (last.classList.contains('authorization-page-break')) { last.remove(); continue; }
      if (children.length === 1) {
        if (splitElement(last, page, nextPage)) continue;
        page.removeChild(last); nextPage.insertBefore(last, nextPage.firstChild); continue;
      }
      page.removeChild(last); nextPage.insertBefore(last, nextPage.firstChild);
    }
  }

  function handleExplicitBreaks(pages) {
    for (let i = 0; i < pages.length; i += 1) {
      const page = pages[i], marker = Array.from(page.children).find((el) => el.classList.contains('authorization-page-break'));
      if (!marker) continue;
      let next = pages[i + 1];
      if (!next) { next = makePage(); page.parentNode.insertBefore(next, page.nextSibling); pages.splice(i + 1, 0, next); }
      const children = Array.from(page.children), markerIndex = children.indexOf(marker);
      children.slice(markerIndex + 1).forEach((node) => next.appendChild(node)); marker.remove();
    }
  }
  ...
  function paginate() {
    ...
    try {
      ...
      pages.forEach(pageStyle); handleExplicitBreaks(pages);
      for (let i = 0; i < pages.length; i += 1) {
        const page = pages[i]; let next = pages[i + 1];
        if (!next) { next = makePage(); editor.appendChild(next); pages.push(next); }
        moveOverflow(page, next);
        if (!next.children.length && i === pages.length - 2) { next.remove(); pages.pop(); }
      }
      pages = allPages();
      pages.forEach((page, index) => { page.dataset.pageNumber = String(index + 1); pageStyle(page); });
      restoreSelection(editor, savedSelection);
      if (typeof window.saveLetter === 'function') window.saveLetter();
    } finally { paginating = false; }
  }

  function schedulePagination() {
    clearTimeout(paginationTimer); paginationTimer = window.setTimeout(() => requestAnimationFrame(paginate), 40);
  }
  ...
  function overrideLetterPdfExport() {
    ...
    const exportFn = async function () {
      paginate();
      const editor = getEditor(), clone = editor.cloneNode(true);
      clone.style.width = `${SPEC.w}px`; clone.style.height = 'auto'; clone.style.minHeight = '0'; clone.style.maxHeight = 'none'; clone.style.overflow = 'visible'; clone.style.padding = '0';
      clone.querySelectorAll('.authorization-page').forEach((page) => { page.style.width = `${SPEC.w}px`; page.style.height = `${SPEC.h}px`; page.style.minHeight = `${SPEC.h}px`; page.style.maxHeight = `${SPEC.h}px`; page.style.overflow = 'hidden'; page.style.margin = '0'; page.style.boxShadow = 'none'; page.style.border = '0'; page.style.breakAfter = 'page'; page.style.pageBreakAfter = 'always'; });
      const wrapper = document.createElement('div'); wrapper.style.background = '#fff'; wrapper.style.padding = '0'; wrapper.style.width = `${SPEC.w}px`; wrapper.style.height = 'auto'; wrapper.appendChild(clone);
      const root = document.createElement('div'); root.style.position = 'fixed'; root.style.left = '-100000px'; root.style.top = '0'; root.style.width = `${SPEC.w}px`; root.style.background = '#fff'; root.appendChild(wrapper); document.body.appendChild(root);
      try { return await window.html2pdf().set({ margin: 0, filename: 'AUTHORIZATION.pdf', image: { type: 'jpeg', quality: .97 }, html2canvas: { scale: 2, backgroundColor: '#fff', useCORS: true }, pagebreak: { mode: ['css', 'legacy'] }, jsPDF: { unit: 'pt', format: SPEC.pdf, orientation: 'portrait' } }).from(wrapper).outputPdf('blob'); }
      finally { root.remove(); }
    };
    exportFn.__pagedOverride = true; exportFn.__original = original; window.createLetterPdfBlob = exportFn;
  }

  function mount() {
    const editor = getEditor(); if (!editor) return;
    paginate(); editor.addEventListener('input', schedulePagination); window.addEventListener('resize', schedulePagination);
    const observer = new MutationObserver(() => { if (!paginating) schedulePagination(); }); observer.observe(editor, { childList: true, subtree: true });
    window.setTimeout(overrideLetterPdfExport, 0); window.setTimeout(overrideLetterPdfExport, 250);
  }
